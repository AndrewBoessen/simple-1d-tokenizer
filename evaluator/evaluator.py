...
def get_covariance(sigma, total, num_examples):
    return (
        ...
        if num_examples == 0
        else (sigma - torch.outer(total, total) / num_examples) / (num_examples - 1)
    )
    ...
    def update(self, real_images, fake_images, codebook_indices=None):
        ...
        if self._enable["inception_score"] or self._enable["rfid"]:
            ...
            if self._enable["inception_score"]:
                ...
                self._is_total_kl_d += torch.sum(
                    probs * torch.log(probs + self._is_eps), 0, dtype=torch.float64
                )
    ...
    def result(self):
        ...
        if self._enable["inception_score"]:
            ...
            avg_kl_d = (
                torch.sum(
                    self._is_total_kl_d
                    - self._is_prob_total * torch.log(mean_probs + self._is_eps)
                )
                / self._num_examples
            )
        ...
        if self._enable["rfid"]:
            ...
            tr_covmean = np.trace(covmean.real if np.iscomplexobj(covmean) else covmean)
        ...
        if self._enable["codebook_entropy"]:
            ...
            eval_score["CodebookEntropy"] = (-torch.log2(probs + 1e-8) * probs).sum()

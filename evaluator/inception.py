...
class FeatureExtractorInceptionV3(FeatureExtractorBase):
    ...
    def forward(self, x):
        ...
        vassert(x.dim() == 4 and x.shape[1] ==
                3, f'Input is not Bx3xHxW: {x.shape}')
        features = {}
        ...
        if '768' in remaining_features:
            features['768'] = F.adaptive_avg_pool2d(x, output_size=(
                1, 1)).squeeze(-1).squeeze(-1).to(torch.float32)
...
def get_inception_model():
    model = FeatureExtractorInceptionV3(
        "inception_model", ["2048", "logits_unbiased"])
    return model
